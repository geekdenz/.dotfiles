...
Item {
  ...
  // Show toasts only on the centre/main display and keep at most three
  // visible. New arrivals evict the oldest toast from the bottom.
  readonly property string popupScreenName: "DP-2"
  ...
  function enforcePopupLimit() {
    while (popupModel.count > popupLimit)
      removePopup(popupModel.count - 1, "limit")
  }
  ...
  Variants {
    model: (Quickshell.screens || []).filter(function(candidate) {
      return candidate && candidate.name === service.popupScreenName
    })
        ...
        Repeater {
          ...
          delegate: Item {
            ...
            NotificationCard {
              ...
              // Clicking a toast only dismisses it. Do not invoke the sender's
              // default action or focus one of its windows.
              onCardClicked: service.dismissPopup(cardSlot.index)
            }
          }
        }
      }
