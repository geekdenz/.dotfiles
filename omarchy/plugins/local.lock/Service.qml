...
Item {
  ...
  Timer {
    ...
    // The stock service gives up after about ten seconds. External monitors can
    // remain asleep much longer, so keep checking slowly until Hyprland can
    // answer. Once the session is known locked or unlocked, this timer stops.
    interval: 2000
    repeat: true
    running: !root.strandedLockResolved

    function rearm() {
      if (!root.strandedLockResolved) restart()
    }
